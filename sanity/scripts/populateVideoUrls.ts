 ...
 */
async function findTutorialUrl(exerciseName: string, apiKey: string): Promise<string | null> {
    // Step 1 — Search (100 units)
    const searchUrl = new URL('https://www.googleapis.com/youtube/v3/search')
    searchUrl.searchParams.set('key', apiKey)
    searchUrl.searchParams.set('part', 'id')
    searchUrl.searchParams.set('type', 'video')
    searchUrl.searchParams.set('q', `${exerciseName} exercise tutorial proper form`)
...
async function runFetch() {
    ...
    const allExercises = await readClient.fetch<Array<{ _id: string; name: string }>>(
        `*[_type == "exercise"] { _id, name } | order(name asc)`,
    )
    ...
    for (let i = 0; i < todo.length; i++) {
        ...
        try {
            const videoUrl = await findTutorialUrl(ex.name, YOUTUBE_API_KEY)
